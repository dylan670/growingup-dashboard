...
BRAND_RULES: list[tuple[list[str], str]] = [
    ...
    (["롤라루"], "롤라루"),
    (["캐리어", "백팩", "여행"], "롤라루 여행용품"),
    # 루티니스트 — 러닝/스포츠/다이어리 제품군
    (["루티니스트", "Routinist", "ROUTINIST"], "루티니스트"),
    (["런닝", "러닝", "조깅", "트레이닝"], "루티니스트 러닝"),
    (["다이어리", "플래너", "노트", "저널", "스케줄러"], "루티니스트 다이어리"),
]
...
PRODUCT_NAME_RULES: list[tuple[str, str]] = [
    ...
    # ---- 루티니스트 다이어리 제품 (스마트폰 오매칭 방지 용) ----
    ("스마트폰 거치 만년다이어리", "만년다이어리"),
    ("만년다이어리", "만년다이어리"),
    # ---- 루티니스트 러닝 제품 — 짧고 통일된 모델명 ----
    # 'X → Y' 패턴: 상품명에 포함된 키워드 → canonical 모델명
    # 주문/캐시 양쪽에 동일 규칙 적용되어 매칭률 향상
    ("러닝조끼", "러닝조끼"),                 # 모든 러닝조끼 variant 통일
    ("러닝 장갑", "러닝 장갑"),
    ("러닝장갑", "러닝 장갑"),
    ("손목밴드", "손목밴드"),
    ("헤어밴드", "헤어밴드"),
    # ---- 짧은 키워드 fallback (오타 보정 + 모델명 표준화) ----
    # ⚠️ '스마트' 는 브랜드 컨텍스트와 함께 — '스마트폰' 같은 generic 단어 오매칭 방지
    ("롤라루 스마트", "스마트"),
    ("스마트 캐리어", "스마트"),
    # 나머지는 브랜드 고유 모델명이라 단독 매칭 안전
    ("큐포이드", "큐보이드"),          # 오타 보정
    ("큐보이드", "큐보이드"),
    ("스파클링", "스파클링"),
    ("플렉스", "플렉스"),
...
BRAND_ORDER_STORES: dict[str, list[str]] = {
    ...
    # 루티니스트: 네이버 스마트스토어 + Cafe24 자사몰 (API 연동)
    "루티니스트": ["루티니스트", "자사몰_루티니스트"],
}
...
BRAND_AD_STORES: dict[str, list[str]] = {
    ...
    "루티니스트":  [
        "자사몰_루티니스트",   # Meta 광고 (Routinist 자사몰 광고 계정)
    ],
}
...
BRAND_STORE_MONTHLY_TARGETS: dict[str, dict[str, int]] = {
    ...
    "루티니스트": {
        "루티니스트":          1_500_000,    # 네이버 스마트스토어
        "자사몰_루티니스트":   1_500_000,    # Cafe24 자사몰
    },
}
...
BRAND_MONTHLY_TARGETS: dict[str, int] = {
    ...
    "루티니스트": sum(BRAND_STORE_MONTHLY_TARGETS["루티니스트"].values()), # 3,000,000원
}
