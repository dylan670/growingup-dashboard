...
BRAND_AD_STORES: dict[str, list[str]] = {
    # 네이버 검색광고는 캠페인명 기반 자동 분류로 store 가 분리됨:
    #   네이버_똑똑연구소 (KNS_/김똑똑/떡뻥/똑똑연구소 매칭)
    #   네이버_롤라루     (RNS_/롤라루/캐리어/백팩 매칭)
    # 구버전 하위호환을 위해 '네이버' (분류 미적용 데이터) 도 똑똑연구소에 포함
    "똑똑연구소":  ["네이버", "네이버_똑똑연구소", "자사몰_똑똑연구소"],
    "롤라루":      ["네이버_롤라루", "자사몰_롤라루"],
    "루티니스트":  [],  # 광고 데이터 없음
}
