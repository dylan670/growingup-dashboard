...
_STORE_CACHE_FALLBACK: dict[str, list[str]] = {
    ...
    # 자사몰 Cafe24 OAuth 토큰 만료 시 네이버 스마트스토어 + 쿠팡 이미지로 fallback
    # (자사몰과 네이버/쿠팡은 같은 브랜드 상품을 판매하므로 이름 유사도 매칭으로 대체 가능)
    "자사몰_똑똑연구소": ["똑똑연구소", "쿠팡_똑똑연구소", "쿠팡"],
    "자사몰_롤라루":     ["롤라루", "쿠팡_롤라루", "쿠팡"],
    # 루티니스트 — 네이버/자사몰 간 상호 fallback
    "자사몰_루티니스트": ["루티니스트"],
    "루티니스트":        ["자사몰_루티니스트"],
}
