...
<div style="background:white; border:1px solid #e2e8f0; border-radius:14px; padding:16px 18px; min-height:110px; box-shadow:0 1px 2px rgba(15,23,42,0.03);">
    ...
    # 카테고리 분류 함수 — 우선순위 중요!
    # ⚠️ "네임택 증정" 문구가 캐리어 본체 product 에 자주 있어서
    #    "네임택" 키워드 단독으로는 부자재로 분류하지 않음.
    def _categorize(p: str) -> str:
        pn = str(p).replace(" ", "")
        # 1. 김 (똑똑연구소) — "떡뻥" 단어 없는 것만
        if any(k in pn for k in [
            "김똑똑", "어린이김", "조미김", "도시락김", "아기김",
            "맛있는김", "모양김",
        ]) and "떡뻥" not in pn:
            return "김"
        # 2. 떡뻥/쌀과자
        ...
        if any(k in pn for k in ["백팩", "트래블백"]):
            return "백팩"
        # 4. 진짜 부자재 — "바퀴커버" / "러기지택" 만 (네임택 증정 회피)
        if any(k in pn for k in ["바퀴커버", "러기지택"]):
            return "캐리어 부자재"
        # 5. 캐리어 메인 (네임택 증정 문구 포함된 본체 캐리어 포함)
        if any(k in pn for k in [
            "캐리어", "여행", "기내용", "수화물", "기내반입",
            ...
            return "캐리어"
        # 6. 캐리어 네임택 액세서리 — 01./02. 번호 prefix + 동물 이름
        if (pn.startswith("01.") or pn.startswith("02.")
                or pn.startswith("03.") or pn.startswith("04.")
                or any(k in pn for k in [
                    "헬로판다", "궁금당근", "바나나트리오", "상큼딸기",
                    "스낵버거", "핑크도넛", "스윗아이스", "해피아보카도",
                    "마이백", "럭키코인", "리틀백", "잇츠마인",
                ])):
            return "캐리어 네임택"
        # 7. 구름쿠션 단독 (캐리어 키워드 없이 부자재만)
        if "구름쿠션" in pn:
            return "캐리어 부자재"
        # 8. 러닝조끼 (루티니스트)
        if any(k in pn for k in [
